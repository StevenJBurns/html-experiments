...
var	g_expl = function()
  ...
  this.cycle = function() {this.r -= 0.1;}
  }
...
dist = function(pl, dps) {return (Math.sqrt(Math.pow(pl.x - dps.x, 2) + Math.pow(pl.y - dps.y, 2)));};

dist_ = function(a, b) {return (Math.sqrt(Math.pow(a - b, 2)));};
...
loop = function()
  ...
  for (var i = 0; i < nb_ball; i++)
    ...
    if (A_ball[i])
      ...
      context.lineTo(A_ball[i].cos * 30 + A_ball[i].x, A_ball[i].sin * 30 + A_ball[i].y);
      context.stroke();
      }
    ...
    for (var i = 0; i < A_expl.length; i++)
      ...
      if (A_expl[i])
        ...
        context.closePath();
        A_expl[i].cycle();
        if (A_expl[i].r <= 0) delete A_expl[i];
        }
      }
      ...
      for (var i = 0; i < nb_bps; i++)
        ...
        setTimeout(function() {loop();},1000/FPSMAX);
      }
...
init = function ()
  ...
  for (var i = 0; i < nb_bps; i++)
    {
    tmp = new g_bps;
    tmp.m = Math.floor(Math.random() * 16) + 20;
    tmp.r = tmp.m / 12;
  ...
  for (var i = 0; i < nb_ball; i++)
    {
    tmp = new g_ball;
    tmp.m = Math.floor(Math.random() * 7) + 2;
    tmp.r = tmp.m / 2
    tmp.x = Math.floor(Math.random() * canvas.width) % (canvas.width - 200) + 100;
    tmp.y = Math.floor(Math.random() * canvas.height) % (canvas.height - 200) + 100;
    tmp.vx = 0;
